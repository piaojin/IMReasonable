...
- (void)setCurIndex:(NSInteger)curIndex{
    ...
    if (_imgSource.count) {
        if ([_imgSource[0] isKindOfClass:urlCls]) {
            [self setAllImgVContentFromImage:[self imgListURLFromIndex:_curIndex] withindex:curIndex];
        }else{
            [self setAllImgVContentFromImage:[self imgListFromIndex:_curIndex] withindex:curIndex];
        }
    }
}
...
//            [vRight setImageWithURL:imgList[MRImgLocationRight]];
            ...
            MessageModel * leftmodel=imgList[MRImgLocationLeft];
            MessageModel * centermodel=imgList[MRImgLocationCenter];
            MessageModel * rightmode=imgList[MRImgLocationRight];
            
            NSString *  leftImageURL1 = [leftmodel.content stringByReplacingOccurrencesOfString:@"Small" withString:@""];
            NSString *leftpath=[Tool Append:IMReasonableAPPImagePath witnstring:leftImageURL1];
            NSString *  centerImageURL1 = [centermodel.content stringByReplacingOccurrencesOfString:@"Small" withString:@""];
            NSString *centerpath=[Tool Append:IMReasonableAPPImagePath witnstring:centerImageURL1];
            
            NSString *  rightImageURL1 = [rightmode.content stringByReplacingOccurrencesOfString:@"Small" withString:@""];
            NSString *rightpath=[Tool Append:IMReasonableAPPImagePath witnstring:rightImageURL1];
         
            
             UIImage * lefttempimg=[UIImage imageWithContentsOfFile:[Tool getFilePathFromDoc:leftmodel.content]];
             UIImage * centertempimg=[UIImage imageWithContentsOfFile:[Tool getFilePathFromDoc:centermodel.content]];
             UIImage * righttempimg=[UIImage imageWithContentsOfFile:[Tool getFilePathFromDoc:rightmode.content]];
            
            [vLift  sd_setImageWithURL:[NSURL URLWithString:leftpath] placeholderImage:lefttempimg];
            [vCenter  sd_setImageWithURL:[NSURL URLWithString:centerpath] placeholderImage:centertempimg];
            [vRight  sd_setImageWithURL:[NSURL URLWithString:rightpath] placeholderImage:righttempimg];
...
//            [vRight setImage:imgList[MRImgLocationRight]];
            ...
            //设置显示的每张图片的title
            __block   UILabel * lab =(UILabel*)[self.superview viewWithTag:989];
            NSInteger index0=1;
            lab.text=[NSString stringWithFormat:@"%ld/%lu",(long)((index+1)>_imgSource.count?index0:(index+1)),(unsigned long)_imgSource.count];
