...
- (void)handleForwardCell:(id)sender{//转发
    NSLog(@"handleForwardcell");
    [self.delegate acForwardMessage:_messagemode];
}
-(void)handleSavePhotoCell:(id)sender{//保存图片到相册
    ...
    [manage downloadImageWithURL:url options: SDWebImageContinueInBackground progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL){
...
//        
//        
//        UIImage * tempimg=[UIImage imageWithContentsOfFile:[Tool getFilePathFromDoc:_messagemode.content]];//[UIImage imageWithContentsOfFile:[Tool
//        
//        //自己发送的就是本地图片
//        if (tempimg) {
...
//                
//            }
//        _pictureContent.frame=CGRectMake(_ScreenWidth-USERPHOTOOFFSET-USERPHOTOSIZE-imagewidth-15-2.5-5, USERPHOTOOFFSET/2, imagewidth+15+2.5,imageheight+2);
//            
//            if ([_messagemode.isNeedSend isEqualToString:@"0"]&&[_messagemode.isReceived  isEqualToString:@"0"]) {
//                reSend=[[UIImageView alloc] init];
//                [self setGesRec];
...
//    }else{
//        //计算名字的宽度
//        UIFont *font = [UIFont systemFontOfSize:MESSAGECONNECTSIZE];
//        NSMutableParagraphStyle *paragraphStyle = [[NSMutableParagraphStyle alloc]init];
//        paragraphStyle.lineBreakMode = NSLineBreakByWordWrapping;
//        NSDictionary *attributes = @{NSFontAttributeName:font, NSParagraphStyleAttributeName:paragraphStyle.copy};
...
//        
//        
//        CGFloat width=namesize.width>140?namesize.width:140;
//        
//        width+=15;
//        
//        _userPhoto.frame=CGRectMake(USERPHOTOOFFSET, USERPHOTOOFFSET, USERPHOTOSIZE, USERPHOTOSIZE);
//        
//        _pictureContent.frame=CGRectMake(USERPHOTOOFFSET+USERPHOTOSIZE+5, USERPHOTOOFFSET/2, width, 140+20+5);
//        
//    }
//    
//    [_pictureContent setMessagemode:self.messagemode];
...
//            namesize = [self.messagemode.username boundingRectWithSize:CGSizeMake(MAXFLOAT, 20) options:NSStringDrawingUsesLineFragmentOrigin attributes:attributes context:nil].size;
        ...
        CGFloat width=140;//namesize.width>140?namesize.width:140;
